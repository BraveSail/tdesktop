...
bool OverlayWidget::hasCopyMediaRestriction(bool skipPremiumCheck) const {
	if (const auto story = _stories ? _stories->story() : nullptr) {
		...
		return !_message->allowsMediaDownloadControls();
	}
...
void OverlayWidget::documentUpdated(not_null<DocumentData*> document) {
	if (_document != document) {
		...
		if (_message
			&& !_message->allowsMediaDownloadControls()
...
void OverlayWidget::fillContextMenuActions(
	...
	if ((!story || story->canDownloadChecked())
		...
		&& (!_message || _message->allowsMediaDownloadControls())) {
...
void OverlayWidget::draw() {
	if (!_session || _stories || showCopyMediaRestriction()) {
		return;
	}
...
bool OverlayWidget::copyRecognitionSelection() {
	...
	if (text.isEmpty()) {
		return false;
	} else if (showCopyMediaRestriction()) {
		return true;
	}
